 ...
 */
 ...
 #define HCI_VSC_SET_DEV_ADDR_LENGTH   	  (6u)
 ...
 static int bt_update_local_dev_addr(const struct device *bt_uart_dev)
 {
	 struct net_buf *buf;
	 int err;
	 uint8_t hci_data[HCI_VSC_SET_DEV_ADDR_LENGTH];
	 uint8_t *local_addr = local_dev_addr(); 

	 /* Baudrate is loaded LittleEndian */
	 for (int i = 0; i < HCI_VSC_SET_DEV_ADDR_LENGTH; i++) {
		 hci_data[i] = local_addr[5-i];
	 }
 
	 buf = bt_hci_cmd_create(BT_HCI_VND_OP_SET_LOCAL_DEV_ADDR,
		 HCI_VSC_SET_DEV_ADDR_LENGTH);
	 if (buf == NULL) {
		 LOG_ERR("Unable to allocate command buffer");
		 return -ENOMEM;
	 }
	 ...
	 /* Add data part of packet */
	 net_buf_add_mem(buf, &hci_data, HCI_VSC_SET_DEV_ADDR_LENGTH);
 ...
 int bt_h4_vnd_setup(const struct device *dev)
 {
	 ...
	 err = bt_update_local_dev_addr(dev);
	 if (err) {
	 	return err;
	 }
